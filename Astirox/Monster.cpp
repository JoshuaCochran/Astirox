...
void Monster::update_map_pos(sf::Vector2f previous, sf::Vector2f target)
{
	Game::currentMap->monster_moved(previous, target);
}
...
void Monster::Update(Map& map)
{
	...
	{
		map.Astar(GetPosition(), Game::player->GetPosition(), *this);
		int count = 0;
		while (!move_path.empty() && count < 2)
		{
			move(move_path.top()->pos.x - GetPosition().x, move_path.top()->pos.y - GetPosition().y);
